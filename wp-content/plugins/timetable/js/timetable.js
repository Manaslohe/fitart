jQuery(document).ready(function($) {
    ...
    $(".tt_tabs").tabs({
        ...
        create: function(event, ui) {
            ...
            if ($timetable.find(".tt_tabs_navigation.all_filters").length && window.location.href.indexOf("book-event-hour-") === -1) {
    ...
    $(".tt_tabs_navigation:not(.all_filter) a, .tabs_box_navigation:not(.all_filter) a").click(function(event) {
        event.preventDefault();
        var $this = $(this),
            hash, newHref, sharpIdx, event_str, events_category_str, all_events_str;
    ...
    $(".tt_tooltip").bind("mouseover click", function() {
        var $this = $(this),
            $attach_to = $this,
            $tooltip_text, position, top, left;
    ...
    function handle_orinentation_change() {
        var no_change_count_to_end = 100,
            no_end_timeout = 1000;
        (function() {
            var orientationchange_interval, orientationchange_timeout, end_timming, curr_inner_width, curr_inner_height, last_inner_width, last_inner_height, no_change_count;
    ...
    $(document.body).on('click', '.tt_guest_option a', function(event) {
        ...
        $(this).parent().addClass('tt_hide').next('.tt_login_option').removeClass('tt_hide');
